		...
		另以下表单根据类的名称进行相应的修改
		 ...
		 -->
			...
			<div class="layui-form-item">
				...
				<div class="layui-input-block">
				<select lay-filter="base_id" name="role.id">
					<option  <c:if test="${user.role == null }">selected</c:if> value="0">-- 无用户组 --</option>
					<c:forEach items="${requestScope.roles}" var="p">
					<option  <c:if test="${user.role.id - p.id == 0 }">selected</c:if> value="${p.id }">${p.name }</option>
					</c:forEach>
				</select>
			...
			<div class="layui-form-item">
				...
				<div class="layui-input-block">
					<form:input path="mobile" placeholder="手机号码" autocomplete="off" class="layui-input" /> <form:errors path="email"></form:errors>
