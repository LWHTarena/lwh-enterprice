...
layui.use('table', function(){
  ...
  table.render({
    elem: '#demo'
   // ,height: 315
    ,url: '${requestScope.marketUrl}/action_templet_market/list' //数据接口
    ,page: true //开启分页
    ,limit: ${requestScope.pageSize}
    
    ,cols: [[ //表头
      {type:'numbers', title: 'ID',  fixed: 'left', width: 60,align:'center'}
      ,{field: 'name', title: '名称', templet: '#titleTpl',minWidth:280}
      ,{field: 'free', title: '性质', templet: '#freeTpl',width:60}
      ,{field: 'author', title: '作者',width:160}
      ,{field: 'downs', title: '人气', width: 60,align:'center'}
      ,{field: 'designTime', title: '创建时间', width: 165}
      ,{field: 'modifyTime', title: '修改时间', width: 165}
...
<script type="text/html" id="titleTpl">
  <a href="${pageContext.request.contextPath}/action_portal_templet/remote/{{d.id}}" class="layui-table-link">{{d.name}}</a>
</script>
<script type="text/html" id="freeTpl">
  {{#  if(d.free == 1){ }}
<span class="free">免费</span>
  {{#  } else { }}
<span class="fee">收费</span>
  {{#  } }}
</script>
 <script lang='javascript' type='text/javascript'> 
