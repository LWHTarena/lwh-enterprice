...
<blockquote class="layui-elem-quote layui-quote-nm"> >> <i class="layui-icon">&#xe61d;</i> 资源文件列表</blockquote>
 <c:if test="${empty requestScope.rs.list }">
 <div class="nullSoftware">没有日志。</div>
...
<table class="layui-table" lay-filter="demo">
  <thead>
   ...
   <tbody>
    ...
    <c:forEach items="${requestScope.rs.list }" var="resFile" varStatus="st">
    

					<tr>
						<td>${(rs.page-1)*rs.pageSize+st.index+1}</td>
						<td>${resFile.filename }</td>
						<td><date:date value ='${resFile.lastModified }' fmtstr='yyyy-MM-dd HH:mm:ss' /></td>
						<td><fmt:formatNumber var="intSize" pattern="#,###,###,###" value="${resFile.size/1024  + 0.5}" /> ${intSize} KB</td>
						
						<td><a title="查阅" href="javascript:details('${resFile.realpath }','${resFile.filename }')"> <i class="layui-icon">&#xe615;</i> </a><!--  <a title="删除" class="delete" href="javascript:del('${resFile.realpath }','${resFile.filename }')"> <i class="layui-icon">&#xe640;</i> </a>--></td>
 ...
 <script lang='javascript' type='text/javascript'> 
	...
	*/		
			...
			function details(realpath,name) {
				//var fileExtend = name.match(//.[a-zA-Z]+$/);
				var fileExtend = name.substring(name.lastIndexOf('.'), name.length).toLowerCase();
				//alert(name+" fileExtend:"+fileExtend);
				if (fileExtend==".res"){
					//alert(realpath);
					parent.layer.open({
					    type: 2,
					    skin: 'layui-layer-rim', //加上边框
						title: '查阅资源文件 “'+name+'” 的内容',
					    area : ['960px' , '598px'],
					    content: '${pageContext.request.contextPath}/action_res/details?securityStr='+realpath,
					...
					parent.layer.msg('当前文件类型不可以查看文件内容！');
				}
				
				
			}

			function del(realpath,name){
				/* parent.layer.confirm('您确定删除资源文件  “'+name+'” 吗？ 删除后将不可恢复！',{title:'警告'}, function(index){
					  
					  var uri="${pageContext.request.contextPath}/action_res/del?securityStr="+realpath;
