...
<blockquote class="layui-elem-quote layui-quote-nm"> >> <i class="layui-icon">&#xe613;</i> 管理员列表　　<c:if test="${requestScope.admin0 }"><a title="增加" href="javascript:add();"> <i class="layui-icon" style="font-size: 18px;">&#xe61f;</i> </a></c:if></blockquote>
 <c:if test="${empty requestScope.rs.list }">
 <div class="nullSoftware">没有任何管理员。</div>
...
<table class="layui-table" lay-filter="demo">
  <thead>
    <tr>
		...
		<th>上次登录时间/IP</th>
		<th>状态</th>
		<c:if test="${requestScope.admin0 }">
		<th>操作</th>
		</c:if>
    </tr>
   </thead> 
   <tbody>
    <c:forEach items="${requestScope.rs.list }" var="admin" varStatus="st">
					...
					<tr>
						...
						<td>${admin.email }</td>
						<td><date:date value ='${admin.createTime }' fmtstr='yyyy-MM-dd' /> / ${admin.createIP } </td>
						<td><date:date value ='${admin.lastLoginTime }' fmtstr='yyyy-MM-dd' /> / ${admin.lastLoginIP } </td>
						<td><c:if test="${admin.state }">正常</c:if><c:if test="${admin.state==false }"><font color="red">禁用</font></c:if></td>
						<c:if test="${requestScope.admin0 }">
						<td><a title="编辑" href="javascript:details(${admin.id },'${admin.username }')"> <i class="layui-icon">&#xe642;</i> </a>　<a title="修改密码" href="javascript:pws(${admin.id },'${admin.username }')"> <i class="layui-icon">&#xe620;</i> </a>　<a title="删除" class="delete" href="javascript:del(${admin.id },'${admin.username }')"> <i class="layui-icon">&#xe640;</i> </a></td>
						</c:if>
					</tr>
	</c:forEach>
  </tbody>
 ...
 <script lang='javascript' type='text/javascript'> 
	...
	*/		
			...
			function add() {
				...
				title: '新增：请输入管理员信息',
				    area : ['700px' , '500px'],
				    content: '${pageContext.request.contextPath}/jsp/<spring:message code="folder.admin"></spring:message>/add',
			...
			function details(id,name) {
				layer.open({
				    type: 2,
				    skin: 'layui-layer-rim', //加上边框
				title: '修改：请输入管理员 “'+name+'” 的信息',
				    area : ['700px' , '600px'],
				    content: '${pageContext.request.contextPath}/action_admin/edit?id='+id,
				    success: function(layero, index){
				    },
				    end: function(){
				    	location.reload();
			...
			function del(id,name){
				parent.layer.confirm('您确定删除管理员  “'+name+'” 吗？ 删除后将不可恢复！',{title:'警告'}, function(index){
					  
					  var uri="${pageContext.request.contextPath}/action_admin/del?id="+id;
			...
			function pws(id,username){
				layer.prompt({
					  ...
					  area: ['800px', '350px'] //自定义文本域宽高
						...
						uri ='${pageContext.request.contextPath}/action_admin/pws?uid='+id+"&password=" + value;
						  ...
						  $.ajax({
								...
								success : function(data) {
									...
									if (data== 0){
										...
										layer.msg('<i class="layui-icon">&#x1007;</i> 操作失败，请检查您是否有权限或重新登录后操作！');
										//alert("修改时发生错误！");
									}
								}
							}); 
